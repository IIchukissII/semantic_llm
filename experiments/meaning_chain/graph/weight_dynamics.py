"""
Weight Dynamics Module - Learning and Forgetting for Semantic Edges
====================================================================
...
LEARNING_INCREMENT = 0.05  # w += 0.05 per reinforcement (matches existing code)
...
def learn_weight_simple(w_current: float,
                        ...
                        w_max: float = W_MAX) -> float:
    """
    Apply simple additive learning (matches existing code).

    Formula: w(t+1) = min(w_max, w_current + increment)

    This is the simpler approach currently used in conversation_learner.py.
...
def compute_decay_batch(weights: list, days_elapsed: float) -> list:
    """
...
def decay_statistics(weights_before: list, weights_after: list) -> dict:
    """
